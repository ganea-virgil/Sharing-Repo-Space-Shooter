...
def main():
    ...
    yellow = pygame.Rect(WIDTH//4 - SPACESHIP_WIDTH//2, HEIGHT//2 - SPACESHIP_HEIGHT//2, SPACESHIP_HEIGHT, SPACESHIP_WIDTH)     # I swapped SPACESHIP_HEIGHT and height to compensate for the fact the we rotated the models 90 degrees
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LCTRL and len(yellow_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(
                             yellow.x + yellow.width, yellow.y + yellow.height // 2 - 3, 13, 6)
                    yellow_bullets.append(bullet)
                if event.key == pygame.K_RCTRL and len(red_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(
                             red.x - 10, red.y + red.height // 2 - 3, 13, 6)
